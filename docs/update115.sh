...
echo "This will update Namingo Registrar from v1.1.4 to v1.1.5"
...
echo "Cloning v1.1.5 from the repository..."
git clone --branch v1.1.5 --single-branch https://github.com/getnamingo/registrar /opt/registrar115
...
copy_files "/opt/registrar115/automation" "/opt/registrar/automation"
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar115..."
    rm -rf /opt/registrar115
    rm -rf /opt/registrar114
else
    echo "There was an issue starting the services. /opt/registrar115 will not be deleted."
fi

# Final instructions to the user
echo
echo "=== Upgrade to v1.1.5 Complete ==="
echo
