...
echo "This will update Namingo Registrar from v1.2.0 to v1.2.1"
...
if [[ -s /var/www/di.php ]]; then
    ...
    echo
    echo "Namingo Registrar v1.2.1 includes FOSSBilling v0.8.5"
...
if systemctl is-active --quiet caddy; then
    ...
    echo "Neither caddy, nginx nor apache2 is running."
fi
systemctl stop whois
systemctl stop rdap

# Clone the new version of the repository
echo "Cloning v1.2.1 from the repository..."
git clone --branch v1.2.1 --single-branch https://github.com/getnamingo/registrar /opt/registrar121
...
copy_files "/opt/registrar121/automation" "/opt/registrar/automation"
copy_files "/opt/registrar121/whois" "/opt/registrar/whois"
copy_files "/opt/registrar121/rdap" "/opt/registrar/rdap"
copy_files "/opt/registrar121/tests" "/opt/registrar/tests"
copy_files "/opt/registrar121/docs" "/opt/registrar/docs"
...
# Install YAML extension if missing
if php -m | grep -qi '^yaml$'; then
    echo "PHP YAML extension is already installed."
else
    echo "Installing PHP YAML extension..."
    apt-get update
    apt-get install -y "php${PHP_VERSION}-yaml"
fi
...
if systemctl list-unit-files | grep -q "^${PHP_FPM_SERVICE}\.service"; then
    ...
    echo "WARNING: ${PHP_FPM_SERVICE}.service not found. PHP-FPM was not restarted."
fi
...
if systemctl is-active --quiet caddy || systemctl is-enabled --quiet caddy; then
...
elif systemctl is-active --quiet nginx || systemctl is-enabled --quiet nginx; then
    echo "Starting nginx..."
    ...
    echo "Neither caddy, nginx nor apache2 is active or enabled."
fi
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar121..."
    rm -rf /opt/registrar121
else
    echo "There was an issue starting the services. /opt/registrar121 will not be deleted."
fi

# Final instructions to the user
echo
echo "=== Upgrade to v1.2.1 Complete ==="
echo
