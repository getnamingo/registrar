...
echo "This will update Namingo Registrar from v1.1.2 to v1.1.3"
...
echo "Cloning v1.1.3 from the repository..."
git clone --branch v1.1.3 --single-branch https://github.com/getnamingo/registrar /opt/registrar113
...
copy_files "/opt/registrar113/automation" "/opt/registrar/automation"
copy_files "/opt/registrar113/whois" "/opt/registrar/whois"
copy_files "/opt/registrar113/rdap" "/opt/registrar/rdap"
copy_files "/opt/registrar113/tests" "/opt/registrar/tests"
...
apt install -y ${PHP_VERSION}-xmlrpc
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar113..."
    rm -rf /opt/registrar113
else
    echo "There was an issue starting the services. /opt/registrar113 will not be deleted."
fi

# Final instructions to the user
echo
echo "=== Upgrade to v1.1.3 Complete ==="
echo
