...
echo "This will update Namingo Registrar from v1.2.3 to v1.2.4"
...
echo "Cloning v1.2.4 from the repository..."
git clone --branch v1.2.4 --single-branch https://github.com/getnamingo/registrar /opt/registrar124
...
copy_files "/opt/registrar124/automation" "/opt/registrar/automation"
copy_files "/opt/registrar124/whois" "/opt/registrar/whois"
copy_files "/opt/registrar124/rdap" "/opt/registrar/rdap"
copy_files "/opt/registrar124/tests" "/opt/registrar/tests"
copy_files "/opt/registrar124/docs" "/opt/registrar/docs"
...
# Install APCU extension if missing
if php -m | grep -qi '^apcu$'; then
    echo "PHP APCU extension is already installed."
else
    echo "Installing PHP APCU extension..."
    apt-get update
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar124..."
    rm -rf /opt/registrar124
else
    echo "There was an issue starting the services. /opt/registrar124 will not be deleted."
fi

# Final instructions to the user
echo
echo "=== Upgrade to v1.2.4 Complete ==="
echo
