...
echo "This will update Namingo Registrar from v1.2.2 to v1.2.3"
...
if [[ -s /var/www/di.php ]]; then
    ...
    echo
    echo "Namingo Registrar v1.2.3 includes FOSSBilling v0.8.5"
...
echo "Cloning v1.2.3 from the repository..."
git clone --branch v1.2.3 --single-branch https://github.com/getnamingo/registrar /opt/registrar123
...
copy_files "/opt/registrar123/automation" "/opt/registrar/automation"
copy_files "/opt/registrar123/whois" "/opt/registrar/whois"
copy_files "/opt/registrar123/rdap" "/opt/registrar/rdap"
copy_files "/opt/registrar123/tests" "/opt/registrar/tests"
copy_files "/opt/registrar123/docs" "/opt/registrar/docs"
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar123..."
    rm -rf /opt/registrar123
else
    echo "There was an issue starting the services. /opt/registrar123 will not be deleted."
fi

# Final instructions to the user
echo
echo "=== Upgrade to v1.2.3 Complete ==="
echo
