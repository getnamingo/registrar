...
echo "This will update Namingo Registrar from v1.0.2 to v1.0.3."
...
copy_files "/opt/registrar103/automation" "/opt/registrar/automation"
copy_files "/opt/registrar103/whois" "/opt/registrar/whois"
copy_files "/opt/registrar103/rdap" "/opt/registrar/rdap"
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registrar103..."
    rm -rf /opt/registrar103
else
    echo "There was an issue starting the services. /opt/registrar103 will not be deleted."
fi

# Final instructions to the user
echo "Upgrade to v1.0.3 is almost complete. Please follow the final step below to finish the process:"
echo
